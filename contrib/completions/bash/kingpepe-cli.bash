# bash programmable completion for kingpepe-cli(1)
# Copyright (c) 2012-present The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $kingpepe-cli for RPC
_bitcoin_rpc() {
    # determine already specified args necessary for RPC
    local rpcargs=()
    for i in ${COMP_LINE}; do
        case "$i" in
            -conf=*|-datadir=*|-regtest|-rpc*|-testnet|-signet)
                rpcargs=( "${rpcargs[@]}" "$i" )
                ;;
        esac
    done
...
_bitcoin_cli() {
    ...
    # save and use original argument to invoke kingpepe-cli for -help, help and RPC
    # as kingpepe-cli might not be in $PATH
    kingpepe_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($kingpepe_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi
...
complete -F _bitcoin_cli kingpepe-cli
