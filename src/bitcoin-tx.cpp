...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " kingpepe-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The kingpepe-tx tool is used for creating and modifying Kingpepe transactions.\n\n"
                "kingpepe-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded Kingpepe transaction, or with \"-create [commands]\" to create a hex-encoded Kingpepe transaction.\n"
                "\n"
                "Usage: kingpepe-tx [options] <hex-tx> [commands]\n"
                "or:    kingpepe-tx [options] -create [commands]\n"
                "\n";
