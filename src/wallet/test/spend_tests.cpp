...
BOOST_FIXTURE_TEST_CASE(SubtractFee, TestChain100Setup)
{
    ...
    const CAmount input_amount{3 * COIN};
    const COutPoint selected_input{m_coinbase_txns[1]->GetHash(), 0};
    ...
    auto check_tx = [&wallet, input_amount, selected_input](CAmount leftover_input_amount) {
        CRecipient recipient{PubKeyDestination({}), input_amount - leftover_input_amount, /*subtract_fee=*/true};
        CCoinControl coin_control;
        coin_control.m_feerate.emplace(10000);
        coin_control.fOverrideFeeRate = true;
        coin_control.Select(selected_input);
...
BOOST_FIXTURE_TEST_CASE(wallet_duplicated_preset_inputs_test, TestChain100Setup)
{
    ...
    // Add spendable UTXOs to the wallet.
    ...
    const CAmount target_amount{available_coins.GetTotalAmount() + COIN};
    ...
    //
    // Even with SFFO, the wallet can only afford to send its actual balance.
