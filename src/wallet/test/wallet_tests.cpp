...
BOOST_FIXTURE_TEST_CASE(scan_for_wallet_transactions, TestChain100Setup)
{
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(GetBalance(wallet).m_mine_immature, 2 * 3 * COIN);
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(result.last_scanned_block, newTip->GetBlockHash());
        BOOST_CHECK_EQUAL(*result.last_scanned_height, newTip->nHeight);
        BOOST_CHECK_EQUAL(GetBalance(wallet).m_mine_immature, 3 * COIN);
    }
...
public:
    ...
    {
        coinbaseKey = GenerateRandomKey();
        constexpr int wallet_coinbase_count{static_cast<int>(OUTPUT_TYPES.size()) + 1};
        static_assert(wallet_coinbase_count <= COINBASE_MATURITY);
        for (int i{0}; i < wallet_coinbase_count; ++i) {
            CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()));
        }
        const CKey non_wallet_key{GenerateRandomKey()};
        for (int i{0}; i < COINBASE_MATURITY - wallet_coinbase_count + 1; ++i) {
            CreateAndProcessBlock({}, GetScriptForRawPubKey(non_wallet_key.GetPubKey()));
        }
...
BOOST_FIXTURE_TEST_CASE(ListCoinsTest, ListCoinsTestingSetup)
{
    ...
    // Check initial balance from one mature KingPepe coinbase transaction.
    BOOST_CHECK_EQUAL(3 * COIN, WITH_LOCK(wallet->cs_wallet, return AvailableCoins(*wallet).GetTotalAmount()));
