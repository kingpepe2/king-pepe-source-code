...
struct MinerTestingSetup : public TestingSetup {
    MinerTestingSetup() : TestingSetup{ChainType::REGTEST, {.extra_args = {"-testactivationheight=csv@1000000"}}} {}
    ...
    void SyncKernelTipNotification()
    {
        const uint256 tip_hash{WITH_LOCK(cs_main, return Assert(m_node.chainman)->ActiveChain().Tip()->GetBlockHash())};
        LOCK(m_node.notifications->m_tip_block_mutex);
        m_node.notifications->m_state.tip_block = tip_hash;
        m_node.notifications->m_tip_block_cv.notify_all();
    }
};
} // namespace miner_tests
...
void MinerTestingSetup::TestPackageSelection(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    SetMockTime(Assert(m_node.chainman)->ActiveChain().Tip()->GetBlockTime() + 1);
    ...
    tx.vout.resize(1);
    tx.vout[0].nValue = NORMAL_BLOCK_SUBSIDY - 1000;
    ...
    tx.vout[0].nValue = NORMAL_BLOCK_SUBSIDY - 10000;
    ...
    tx.vout[0].nValue = NORMAL_BLOCK_SUBSIDY - 1000 - 50000; // 50k satoshi fee
    ...
    tx.vout[0].nValue = NORMAL_BLOCK_SUBSIDY - 1000 - 50000; // 0 fee
    ...
    tx.vout[0].nValue = NORMAL_BLOCK_SUBSIDY - 1000 - 50000 - feeToUse;
    ...
    tx.vout.resize(2);
    tx.vout[0].nValue = NORMAL_BLOCK_SUBSIDY - 100000000;
    ...
    tx.vout[0].nValue = NORMAL_BLOCK_SUBSIDY - 100000000 - feeToUse;
...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
    ...
    const CAmount BLOCKSUBSIDY = NORMAL_BLOCK_SUBSIDY;
    const CAmount LOWFEE = CENT;
    const CAmount HIGHFEE = 10 * CENT;
    const CAmount HIGHERFEE = 20 * CENT;
    ...
    tx.vin[0].prevout.n = 0;
    tx.vin[0].scriptSig = CScript() << OP_1;
    tx.vin[0].nSequence = m_node.chainman->ActiveChain().Tip()->nHeight; // txFirst[0] is the 2nd block
    ...
    tx.vin[0].nSequence = CTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG | 1; // txFirst[1] is the 3rd block
    hash = tx.GetHash();
...
void MinerTestingSetup::TestPrioritisedMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.vin[0].scriptSig = CScript() << OP_1;
    tx.vout.resize(1);
    tx.vout[0].nValue = NORMAL_BLOCK_SUBSIDY; // 0 fee
    ...
    tx.vin[0].prevout.hash = hashParentTx;
    tx.vout[0].nValue = NORMAL_BLOCK_SUBSIDY - 1000 - 1000; // 1000 satoshi fee
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    for (const auto& bi : BLOCKINFO) {
        ...
        CMutableTransaction txCoinbase(*block.vtx[0]);
        {
            ...
            if (current_height >= 1 && txFirst.size() == 0)
                baseheight = current_height;
            if (current_height >= 1 && txFirst.size() < 4)
