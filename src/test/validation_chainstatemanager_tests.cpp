...
static bool HasAssumeutxoSnapshot()
{
    return Params().AssumeutxoForHeight(110).has_value();
}
...
{
    if (!HasAssumeutxoSnapshot()) {
        BOOST_TEST_MESSAGE("Skipping assumeutxo activation test: KingPepe chainparams do not define assumeutxo data.");
        return;
    }
...
{
    if (!HasAssumeutxoSnapshot()) {
        BOOST_TEST_MESSAGE("Skipping assumeutxo snapshot-init test: KingPepe chainparams do not define assumeutxo data.");
        return;
    }
...
{
    if (!HasAssumeutxoSnapshot()) {
        BOOST_TEST_MESSAGE("Skipping assumeutxo snapshot-completion test: KingPepe chainparams do not define assumeutxo data.");
        return;
    }
...
{
    if (!HasAssumeutxoSnapshot()) {
        BOOST_TEST_MESSAGE("Skipping assumeutxo hash-mismatch test: KingPepe chainparams do not define assumeutxo data.");
        return;
    }
