...
FUZZ_TARGET(p2p_headers_presync, .init = initialize)
{
    ...
    auto try_add_headers = [&](const auto& headers) {
        ...
        for (const CBlockHeader& header : headers) {
            added_work += GetBlockProof(header);
        }
        if (starting_work + claimed_work + added_work >= min_work) {
            return false;
        }
        claimed_work += added_work;
        return true;
    };
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 100)
    {
        ...
        CallOneOf(
            ...
            [&]() NO_THREAD_SAFETY_ANALYSIS {
                ...
                CBlockHeader next_base{base};
                for (CBlock& header : headers) {
                    header = ConsumeHeader(fuzzed_data_provider, next_base.GetHash(), next_base.nBits);
                    FinalizeHeader(header, chainman);
                    next_base = header;
                }

                if (!try_add_headers(headers)) {
                    return;
                }
                base = next_base;
            ...
            [&]() NO_THREAD_SAFETY_ANALYSIS {
                ...
                const std::vector<CBlockHeader> headers{block};
                if (!try_add_headers(headers)) {
                    return;
                }
    ...
    // This is a conservative overestimate, as base is only moved forward when sending headers.
    const arith_uint256 total_work{starting_work + claimed_work};
