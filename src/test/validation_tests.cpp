...
{
    int maxHalvings = 64;
    CAmount nInitialSubsidy = 3 * COIN;

    for (int nHalvings = 0; nHalvings < maxHalvings; nHalvings++) {
        int nHeight = nHalvings * consensusParams.nSubsidyHalvingInterval;
        CAmount nSubsidy = GetBlockSubsidy(nHeight, consensusParams);
        BOOST_CHECK_EQUAL(nSubsidy, nInitialSubsidy >> nHalvings);
    }
...
BOOST_AUTO_TEST_CASE(block_subsidy_test)
{
    ...
    BOOST_CHECK_EQUAL(GetBlockSubsidy(0, chainParams->GetConsensus()), 3 * COIN);
    BOOST_CHECK_EQUAL(GetBlockSubsidy(1, chainParams->GetConsensus()), 19740000 * COIN);
    BOOST_CHECK_EQUAL(GetBlockSubsidy(2, chainParams->GetConsensus()), 3 * COIN);
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    const auto chainParams = CreateChainParams(*m_node.args, ChainType::MAIN);
    CAmount nSum = 0;
    BOOST_CHECK(MoneyRange(GetBlockSubsidy(1, chainParams->GetConsensus())));
    for (int nHeight = 0; nHeight < 14000000; nHeight += 1000) {
        CAmount nSubsidy = GetBlockSubsidy(nHeight, chainParams->GetConsensus());
        BOOST_CHECK(nSubsidy <= 19740000 * COIN);
        nSum += nSubsidy * 1000;
