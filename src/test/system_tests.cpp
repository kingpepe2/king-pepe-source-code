...
BOOST_AUTO_TEST_CASE(run_command)
{
    ...
    }
    {
        ...
        BOOST_CHECK_EXCEPTION(RunCommandParseJSON(command), std::runtime_error, [&](const std::runtime_error& e) {
            ...
            BOOST_CHECK(what.find("RunCommandParseJSON error: process(") != std::string::npos);
            BOOST_CHECK(what.find(strprintf("returned %d:", boost::exit_test_failure)) != std::string::npos);
            return true;
        });
    }
    {
