...
class Binaries:
    """Helper class to provide information about Kingpepe binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke kingpeped"
        return self._argv("node", self.paths.bitcoind, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke kingpepe-cli"
        # Add -nonamed because "kingpepe rpc" enables -named by default, but kingpepe-cli doesn't
    ...
    def tx_argv(self):
        "Return argv array that should be used to invoke kingpepe-tx"
        return self._argv("tx", self.paths.bitcointx)

    def util_argv(self):
        "Return argv array that should be used to invoke kingpepe-util"
        return self._argv("util", self.paths.bitcoinutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke kingpepe-wallet"
        return self._argv("wallet", self.paths.bitcoinwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke kingpepe-chainstate"
    ...
    def _argv(self, command, bin_path, need_ipc=False):
        ...
        It either uses the kingpepe wrapper executable (if BITCOIN_CMD is set or
        need_ipc is True), or the direct binary path (kingpeped, etc). When
        ...
        wrapper executable directly, not to the commands that `kingpepe` calls.
        """
...
def get_binary_paths(config):
    ...
    binaries = {
        "kingpepe": "BITCOIN_BIN",
        "kingpeped": "BITCOIND",
        "bench_bitcoin": "BITCOIN_BENCH",
        "kingpepe-cli": "BITCOINCLI",
        "kingpepe-util": "BITCOINUTIL",
        "kingpepe-tx": "BITCOINTX",
        "kingpepe-chainstate": "BITCOINCHAINSTATE",
        "kingpepe-wallet": "BITCOINWALLET",
    }
    ...
    # BITCOIN_CMD environment variable can be specified to invoke kingpepe
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Kingpepe"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Kingpepe"
        else:
            datadir = temp_dir / ".kingpepe"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "kingpepe.conf"), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    ...
    if os.path.isfile(os.path.join(datadir, "kingpepe.conf")):
        with open(os.path.join(datadir, "kingpepe.conf"), 'r') as f:
            for line in f:
