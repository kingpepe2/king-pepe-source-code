...
ADDRESS_BCRT1_UNSPENDABLE = 'rkpepe1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqh584mz'
ADDRESS_BCRT1_UNSPENDABLE_DESCRIPTOR = 'addr(rkpepe1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqh584mz)#je4g2vkl'
# Coins sent to this address can be spent with a witness stack of just OP_TRUE
ADDRESS_BCRT1_P2WSH_OP_TRUE = 'rkpepe1qft5p2uhsdcdc3l2uaaap5qqeg4pjaqlp250x7us7a8qqhrxrxfsqts0deq'
...
def create_deterministic_address_bcrt1_p2tr_op_true(explicit_internal_key=None):
    ...
    if explicit_internal_key is None:
        assert_equal(address, 'rkpepe1p9yfmy5h72durp7zrhlw9lf7jpwjgvwdg0jr0lqmmjtgg83266lqslyxvfd')
...
def keyhash_to_p2pkh(hash, main=False):
    assert len(hash) == 20
    version = 51 if main else 64
    return byte_to_base58(hash, version)

def scripthash_to_p2sh(hash, main=False):
    assert len(hash) == 20
    version = 50 if main else 63
    return byte_to_base58(hash, version)
...
def program_to_witness(version, program, main=False):
    ...
    return encode_segwit_address("kpepe" if main else "rkpepe", version, program)
...
def bech32_to_bytes(address):
    ...
    if hrp not in ['kpepe', 'tkpepe', 'rkpepe']:
        return (None, None)
...
def address_to_scriptpubkey(address):
    ...
    if version == 64:  # testnet/regtest pubkey hash
        return keyhash_to_p2pkh_script(payload)
    elif version == 63:  # testnet/regtest script hash
...
class TestFrameworkScript(unittest.TestCase):
    def test_base58encodedecode(self):
        ...
        check_base58(bytes.fromhex('1f8ea1702a7bd4941bca0941b852c4bbfedb2e05'), 64)
        check_base58(bytes.fromhex('3a0b05f4d7f66c3ba7009f453530296c845cc9cf'), 64)
        check_base58(bytes.fromhex('41c1eaf111802559bad61b60d62b1f897c63928a'), 64)
        check_base58(bytes.fromhex('0041c1eaf111802559bad61b60d62b1f897c63928a'), 64)
        check_base58(bytes.fromhex('000041c1eaf111802559bad61b60d62b1f897c63928a'), 64)
        check_base58(bytes.fromhex('00000041c1eaf111802559bad61b60d62b1f897c63928a'), 64)
        check_base58(bytes.fromhex('1f8ea1702a7bd4941bca0941b852c4bbfedb2e05'), 51)
        check_base58(bytes.fromhex('3a0b05f4d7f66c3ba7009f453530296c845cc9cf'), 51)
        check_base58(bytes.fromhex('41c1eaf111802559bad61b60d62b1f897c63928a'), 51)
        check_base58(bytes.fromhex('0041c1eaf111802559bad61b60d62b1f897c63928a'), 51)
        check_base58(bytes.fromhex('000041c1eaf111802559bad61b60d62b1f897c63928a'), 51)
        check_base58(bytes.fromhex('00000041c1eaf111802559bad61b60d62b1f897c63928a'), 51)
