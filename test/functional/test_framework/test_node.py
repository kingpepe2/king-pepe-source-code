...
BITCOIN_PID_FILENAME_DEFAULT = "kingpeped.pid"
...
class TestNode():
    ...
    def __init__(
        ...
        *,
        ...
        self.bitcoinconf = self.datadir_path / "kingpepe.conf"
        ...
        # This means that starting kingpeped using the temp dir to debug a failed test won't
        # spam debug.log.
    ...
    PRIV_KEYS = [
            # address , privkey
            AddressKeyPair('SpaQDwDCKVnSJgmXZiFtmz3V6sS8WFMVnm', 'VPyyhGm4USQ6MV6gL2kpKnZPq7H928fjhWZHW5jCLNvuBR4HJSRo'),
            AddressKeyPair('SxdkTJXPbEbXWfr1sfffYW4jH8T5EHcvCj', 'VP8c5E2o9mNZdcwJ9qTe6QeDW4e3sMm26sF4q9mwYqPEx9r4M7R2'),
            AddressKeyPair('SsvRvFxjmyyzS5AyCMVF5hV9hNw8eKNzDu', 'VN2RftT3mGL2KkQUstCjLxbGUujXR7LPE8YDtRR5sH3pUqx7Ufuk'),
            AddressKeyPair('SvRZYUrarQFjDWLL5Cq3aPFStRoEGuPZKm', 'VQ53sppWCGide5rxZNAqLG3rCNCTm2JLN7wFpiDzcbr2oSNGScUs'),
            AddressKeyPair('SxfEL1RPFcCmKjGAd361AooDdcjBW48B2F', 'VJNvkA2SLCpDkPBLHwfWEP1J1nmR4B5JMgNkN57W3ZofpUcVJdDn'),
            AddressKeyPair('T7ySdNnPyCkfDnCft35whsLSVUSyci9YFy', 'VJkVKv6Fw4riX8iNjJDy6w98XsAME7Zng4oTKFgf1kcR8JymJfdK'),
            AddressKeyPair('T57Z7rRQWBGkUuSAmGjgXFagSiZc9Q3Wev', 'VJXYmb1kkkEasJjjrxgwEETokuceReVKGUXizXvfKrj5dg9y2cRj'),
            AddressKeyPair('SztbBUMMEUTD3WPhBTXPG2ZBDZKYsqjxPA', 'VLhVyaEKDRqJvXjdGyRT3KRVX1pVTzp2MwKF1T8Kg34DPty12H4x'),
            AddressKeyPair('SubmJAS9zGX28K3RxXSjmo4xKgBQAziKVV', 'VLyGSMbaeSsNHCqV2xbES7vPUC18QTEzMpp7YQe925KKGYNFLj5c'),
            AddressKeyPair('SvBJuGd5No7k6fDeauVbyDRTHtJqbtzsRH', 'VGEpBqMptRvLQL3HmsYp1GGSsuq49oPYrPDGV7FumfzWyiUWeYTw'),
            AddressKeyPair('SuMp17iyjr5UEyVnM6Eek6coAtqZADycYf', 'VMHZsNNqYNZdb5gwPz5heATr8oSzL2STCrCDp2xfA14xEgELUHPL'),
            AddressKeyPair('T5YHrJYpHnvtgxRS49z3s5AFg9rhRug1px', 'VHtA4Uq8xMUqBSNUy7SneM8RLQ8meCNcg7BKY36xZQnc849eGxVh'),
    ]
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then kingpeped would bind P2P ports on
        ...
        # kingpepe.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'kingpeped exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; kingpeped may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                latest_error = suppress_error("missing_credentials", e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to kingpeped after {self.rpc_timeout}s (ignored errors: {dict(suppressed_errors)!s}{'' if latest_error is None else f', latest: {latest_error[0]!r}/{latest_error[1]}'})")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        Will raise if kingpeped starts without an error.
        Will raise if an expected_msg is provided and it does not match kingpeped's stdout."""
