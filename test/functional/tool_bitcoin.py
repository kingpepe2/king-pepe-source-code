...
"""Test the KingPepe wrapper tool."""
...
class ToolBitcoinTest(BitcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `kingpepe node -version` is
        # run on windows, python doesn't capture output from the child
        # `kingpeped` and `kingpepe-node` process started with _wexecvp, and
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through the KingPepe wrapper command with specified arguments."""
        # Manually construct the `kingpepe node` command, similar to Binaries::node_argv()
    ...
    def run_test(self):
        ...
        self.log.info("Ensure kingpepe node command invokes kingpeped by default")
        self.test_args([], [], expect_exe="kingpeped")

        self.log.info("Ensure kingpepe -M invokes kingpeped")
        self.test_args(["-M"], [], expect_exe="kingpeped")

        self.log.info("Ensure kingpepe -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure kingpepe -m invokes kingpepe-node")
            self.test_args(["-m"], [], expect_exe="kingpepe-node")

            self.log.info("Ensure kingpepe -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="kingpepe-node")

            self.log.info("Ensure kingpepe accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="kingpepe-node")

            self.log.info("Ensure kingpepe recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="kingpepe-node")
