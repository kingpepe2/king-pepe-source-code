...
class RPCGenerateTest(BitcoinTestFramework):
    ...
    def test_generatetoaddress(self):
        # KingPepe: a valid regtest P2PKH address (node-derived from pkh(<pubkey>));
        # Bitcoin's regtest prefixes do not apply. The second address is a Bitcoin
        # *mainnet* P2SH address which remains invalid on KingPepe regtest.
        self.generatetoaddress(self.nodes[0], 1, 'SwJsa6y3ocWqV47XndRirsqLQPvvbXm3qJ')
    ...
    def test_generateblock(self):
        ...
        # KingPepe regtest bech32 (HRP "rkpepe") P2WPKH for this key (node-derived).
        combo_address = 'rkpepe1qw508d6qejxtdg4y5r3zarvary0c5xw7kg5fvx0'
        ...
        # KingPepe regtest P2PKH for this uncompressed key (node-derived; uncompressed
        # keys cannot use segwit, so the combo descriptor resolves to P2PKH).
        combo_address = 'SqioAMc7RiqZN9gTVNxnJCvL24d1hrudu4'
        hash = self.generateblock(node, 'combo(' + combo_key + ')', [])['hash']
        block = node.getblock(hash, 2)
        assert_equal(len(block['tx']), 1)
        assert_equal(block['tx'][0]['vout'][0]['scriptPubKey']['address'], combo_address)
