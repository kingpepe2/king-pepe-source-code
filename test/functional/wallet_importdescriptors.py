...
class ImportDescriptorsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        addresses = self.nodes[0].deriveaddresses(descsum_create("sh(wpkh(" + xpriv + "/0'/0'/*'))"), [0, 1])
        addresses += self.nodes[0].deriveaddresses(descsum_create("wpkh(" + xpriv + "/0'/0'/*')"), [0, 1])
        ...
        addresses = self.nodes[0].deriveaddresses(descsum_create('wpkh([80002067/0h/0h]' + xpub + '/*)'), [0, 4])
        ...
        expected_address = self.nodes[0].deriveaddresses(descsum_create('pkh([12345678]' + xpub + '/*)'), [0, 0])[0]
        assert_equal(address, expected_address)
        ...
        wif_priv = wif_key.privkey
        address = wif_key.p2sh_p2wpkh_addr
        ...
        test_address(wpriv,
                     ...
                     solvable=True,
                     ismine=True)
        utxo = self.create_outpoints(w0, outputs=[{address: 49.99995540}])[0]
        self.generatetoaddress(self.nodes[0], 6, w0.getnewaddress())
        tx = wpriv.createrawtransaction([utxo], {w0.getnewaddress(): 49.999})
        ...
        multi_priv_receive_desc = "wsh(multi(2," + xprv1 + "/84h/0h/0h/*," + xprv2 + "/84h/0h/0h/*," + xprv3 + "/84h/0h/0h/*))"
        multi_priv_change_desc = "wsh(multi(2," + xprv1 + "/84h/1h/0h/*," + xprv2 + "/84h/1h/0h/*," + xprv3 + "/84h/1h/0h/*))"
        self.test_importdesc({"desc": descsum_create(multi_priv_receive_desc),
                            "active": True,
                            "range": 1000,
                            "next_index": 0,
                            "timestamp": "now"},
                            success=True,
                            wallet=wmulti_priv)
        self.test_importdesc({"desc": descsum_create(multi_priv_change_desc),
                            "active": True,
                            "internal" : True,
                            "range": 1000,
                            "next_index": 0,
        ...
        assert_equal(addr, self.nodes[0].deriveaddresses(descsum_create(multi_priv_receive_desc), [0, 0])[0]) # Derived at m/84'/0'/0'/0
        change_addr = wmulti_priv.getrawchangeaddress('bech32') # uses change 0
        assert_equal(change_addr, self.nodes[0].deriveaddresses(descsum_create(multi_priv_change_desc), [0, 0])[0]) # Derived at m/84'/1'/0'/0
        ...
        multi_pub_receive_desc = "wsh(multi(2,[7b2d0242/84h/0h/0h]" + acc_xpub1 + "/*,[59b09cd6/84h/0h/0h]" + acc_xpub2 + "/*,[e81a0532/84h/0h/0h]" + acc_xpub3 +"/*))"
        multi_pub_change_desc = "wsh(multi(2,[7b2d0242/84h/1h/0h]" + chg_xpub1 + "/*,[59b09cd6/84h/1h/0h]" + chg_xpub2 + "/*,[e81a0532/84h/1h/0h]" + chg_xpub3 + "/*))"
        self.test_importdesc({"desc": descsum_create(multi_pub_receive_desc),
                            "active": True,
                            "range": 1000,
                            "next_index": 0,
                            "timestamp": "now"},
                            success=True,
                            wallet=wmulti_pub)
        self.test_importdesc({"desc": descsum_create(multi_pub_change_desc),
                            "active": True,
                            "internal" : True,
                            "range": 1000,
                            "next_index": 0,
        ...
        assert_equal(addr, self.nodes[0].deriveaddresses(descsum_create(multi_pub_receive_desc), [1, 1])[0]) # Derived at m/84'/0'/0'/1
        change_addr = wmulti_pub.getrawchangeaddress('bech32') # uses change 2
        assert_equal(change_addr, self.nodes[0].deriveaddresses(descsum_create(multi_pub_change_desc), [2, 2])[0]) # Derived at m/84'/1'/0'/2
