...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_dir_config(self):
        ...
        conf_path = self.nodes[0].datadir_path / 'kingpepe.conf'
    ...
    def test_negated_config(self):
        ...
        ignored_file_message = [f'Data directory "{self.nodes[0].datadir_path}" contains a "kingpepe.conf" file which is explicitly ignored using -noconf.']
    ...
    def test_config_file_parser(self):
        ...
        with open(self.nodes[0].datadir_path / 'kingpepe.conf', 'a') as conf:
    ...
    def test_seed_peers(self):
        ...
        # KingPepe has no fixed seed table. Regtest still exercises DNS seed
        # startup, but there is no fixed-seed fallback to log after 60 seconds.
        ...
        # KingPepe has no fixed seeds, so disabling DNS seeds must not add any.
        assert not peer_dat.exists()
        ...
        # With no fixed seeds, the node must not add fixed seeds after the delay.
        assert not peer_dat.exists()
        start = int(time.time())
        with self.nodes[0].assert_debug_log(
                expected_msgs=[
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a kingpepe.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "kingpepe.conf" file which is ignored, because a '
        ...
        # the same kingpepe.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/kingpepe.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when kingpepe.conf in the default data directory sets another datadir '
                      'and it contains a different kingpepe.conf file that would be ignored')
        ...
        # kingpepe.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "kingpepe.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "kingpepe.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/kingpepe.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "kingpepe.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "kingpepe.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_kingpepe.conf"
