...
#  2. the local chain is close to the tip (<144 blocks, about 2.4h for KingPepe)
...
class P2PHandshakeTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that limited peers are only desired if the local chain is close to the tip (<144 blocks)")
        self.generate_at_mocktime(int(time.time()) - 3 * 3600)  # tip outside the 144-block window, should fail
        self.test_desirable_service_flags(node, [NODE_NETWORK_LIMITED | NODE_WITNESS],
                                          DESIRABLE_SERVICE_FLAGS_FULL, expect_disconnect=True)
        self.generate_at_mocktime(int(time.time()) - 2 * 3600)  # tip inside the 144-block window, should succeed
        self.test_desirable_service_flags(node, [NODE_NETWORK_LIMITED | NODE_WITNESS],
