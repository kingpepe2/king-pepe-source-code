...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(balances["mine"]["immature"], Decimal('19740000'))
        ...
        assert_equal(self.nodes[0].getbalance(), Decimal('19740000'))
        assert_equal(self.nodes[1].getbalance(), Decimal('3'))
        ...
        assert_equal(txout['value'], Decimal('19740000'))
        ...
        assert_equal(set([txout1['value'], txout2['value']]), set([Decimal('10'), balance]))
        ...
        # Have node1 generate COINBASE_MATURITY blocks (so node0 can recover the fee)
        self.generate(self.nodes[1], COINBASE_MATURITY, sync_fun=lambda: self.sync_all(self.nodes[0:3]))

        # node0 should end up with the premine plus one 3 KPEPE block reward and fees, but
        # minus the 21 plus fees sent to node2
        assert_equal(self.nodes[0].getbalance(), Decimal('19739982'))
        assert_equal(self.nodes[2].getbalance(), Decimal('21'))
        ...
        assert_equal(self.nodes[2].getbalance(), Decimal('19739997'))
        ...
        node_2_bal = self.check_fee_amount(self.nodes[2].getbalance(), Decimal('19739987'), fee_per_byte, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        # Sendmany 5 KPEPE to two addresses with subtracting fee from both addresses
        ...
        zero_value_source = self.nodes[2]
        usp = zero_value_source.listunspent(query_options={'minimumAmount': '1000'})[0]
        inputs = [{"txid": usp['txid'], "vout": usp['vout']}]
        outputs = {zero_value_source.getnewaddress(): usp['amount'] - Decimal('0.002'), self.nodes[0].getnewaddress(): 11.11}

        raw_tx = zero_value_source.createrawtransaction(inputs, outputs).replace("c0833842", "00000000")  # replace 11.11 with 0.0 (int32)
        signed_raw_tx = zero_value_source.signrawtransactionwithwallet(raw_tx)
        decoded_raw_tx = zero_value_source.decoderawtransaction(signed_raw_tx['hex'])
        zero_value_txid = decoded_raw_tx['txid']
        zero_value_source.sendrawtransaction(signed_raw_tx['hex'])

        self.sync_all()
        self.generate(self.nodes[1], 1)  # mine a block
        node_2_bal -= Decimal('0.002')
        ...
        address_info = self.nodes[0].getaddressinfo("SsmCCfW1WRxwKbteKiJDv9NJP94WChKuot")
        assert_equal(address_info['address'], "SsmCCfW1WRxwKbteKiJDv9NJP94WChKuot")
