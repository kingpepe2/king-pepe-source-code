...
24027 for mainnet, port 61881 for testnet, port 61883 for signet, and port
18443 for regtest.
...
*Query parameters for `verbose` and `mempool_sequence` are available in KingPepe Core v31.1.0.*


Risks
-------------
Running a web browser on the same node with a REST enabled `kingpeped` can be a risk. Accessing prepared XSS websites could read out tx/block data of your node by placing links like `<script src="http://127.0.0.1:24027/rest/tx/1234567890.json">` which might break the node's privacy.
