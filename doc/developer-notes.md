...
configuration options and toggled while `kingpeped` is running with the
`logging` RPC. For instance, launching `kingpeped` with `-debug` or `-debug=1`
will turn on
...
with non-mainnet KingPepe coins on a test network.
...
You can then use the `-debug=lock` configuration option at `kingpeped` startup
or `kingpepe-cli logging '["lock"]'` at runtime to turn on lock contention
logging. It can be toggled off again with
`kingpepe-cli logging [] '["lock"]'`.
...
$ valgrind -v --leak-check=full build/bin/kingpeped -printtoconsole
